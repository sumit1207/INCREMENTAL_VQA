...
class Execution:
    ...
    def train(self, dataset, dataset_eval=None):
        ...
        if self.__C.RESUME:
            ...
            # optim = get_optim(self.__C, net, data_size) if want to start with initial learning rate
        ...
        for epoch in range(start_epoch, self.__C.MAX_EPOCH):
            ...
            # self.__C.CKPT_VERSION for resuming
    ...
    def eval(self, dataset, state_dict=None, valid=False):
        ...
        for step, (
            ...
            rf1.extend(r1.cpu().data.numpy())
            rf2.extend(r2.cpu().data.numpy())
        ...
        data=json.load(open(__C.ANSWER_PATH['val'], 'r'))['annotations']
        an1=[]
        for i in data:
            an1.append((i['answer_type'],i['question_id']))
        with open('outfile_ans', 'wb') as fp:
            pickle.dump(an1, fp)
        with open('outfile_ques', 'wb') as fp:
            pickle.dump(qid_list, fp)
    
        # with open ('outfile', 'rb') as fp:
        #     itemlist = pickle.load(fp)
        np.savez('question_feature.npz',name1=rf1_1,name2=rf2_1)
        print('')
        ...
        if valid:
            ...
            # filename2='/data/ksumit/Incremental/DataSet/vqa-v2/vqa/v2_mscoco_val2014_annotations.json'
            # with open(filename2) as f:
            #     data_ans=json.load(f)['annotations'] 
            # from core.data.data_utils import ques_load

            # with open(filename) as f:
            #     data=json.load(f)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         
            #     questions=data['questions']
            #     qid_to_ques = ques_load(questions)
            #     for i in data_ans:
            #         ques = qid_to_ques[str(i['question_id'])]['question']
            #         tokens = nltk.word_tokenize(ques)
            #         for t in tokens:
            #             if((t in unique_list) and (i['answer_type']=='number')):
            #                 id_q.append(i['question_id'])
            # print("id done")


            # vqaEval.evaluate(id_q)
            vqaEval.evaluate()
